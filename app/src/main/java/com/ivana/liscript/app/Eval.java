...
public class Eval {
     ...
     */
    public static final ConsList emptyList = new ConsList(null, null);
    ...
    public enum SpecialForm {
        ...
        TYPEOF, PRINT, READ, LAMBDA, MACRO, BEGIN, TRACE,
        CLASS, JAVA, FIELD, ACTIVITY, RUNONUI
    }

    /**
     * словарь - список соответствий ключевых слов текста скрипта особым формам языка
     */
    public static final HashMap<String, SpecialForm> specialFormWords = setSpecialFormWords();
    ...
    private static HashMap<String, SpecialForm> setSpecialFormWords() {
        ...
        keyWords.put("field", SpecialForm.FIELD);
        keyWords.put("activity", SpecialForm.ACTIVITY);
        keyWords.put("run-on-ui", SpecialForm.RUNONUI);
...
*/
    ...
    private static boolean paramTypesAreEqual(Class[] neededParamTypes, Class[] paramTypes) {
        if (neededParamTypes.length != paramTypes.length) return false;
        for (int i=0; i<neededParamTypes.length; i++) {
            if (!neededParamTypes[i].isAssignableFrom(paramTypes[i])) return false;
        }
        return true;
    }
        ...
        */
        ...
        for (Constructor cn : cns) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                parameterCount = cn.getParameterCount();
            else
                parameterCount = cn.getParameterTypes().length;

            if (!Modifier.isPublic(cn.getModifiers())
                    || (!cn.isVarArgs() && paramTypes.length != parameterCount)
                    || (paramTypes.length != parameterCount)
            ) continue;
...
//            showParamTypes(cn.getParameterTypes());
//            showParamTypes(paramTypes);
//            if (!Arrays.equals(cn.getParameterTypes(), paramTypes)) continue;
            if (!paramTypesAreEqual(cn.getParameterTypes(), paramTypes)) continue;
            f2.add(cn);
        }
        if (f2.size() == 0)
            throw new RuntimeException("У класса " + c.getName()
    ...
    private static Method methodForName(Class c, String name, Class[] paramTypes)
        ...
        for (Method m : ms) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                ...
                parameterCount = m.getParameterTypes().length;

            if (!Modifier.isPublic(m.getModifiers())
                    || !m.getName().equals(name)
                    || (!m.isVarArgs() && paramTypes.length != parameterCount)
                    || (paramTypes.length > parameterCount)
            ) continue;
...
//    }
    ...
    private static Object getField(Class c, Object o, String name) {
        try {
            Field f = c.getDeclaredField(name);
            return Modifier.isStatic(f.getModifiers()) ? f.get(null) : f.get(o);
        } catch (Throwable e) { throw new RuntimeException(e.getLocalizedMessage(), e); }
    }
...
//            //return ret(d, io, emptyList);
            ...
            if (op instanceof SpecialForm) {
                ...
                switch (sf) {
                    case ACTIVITY:
                        return ret(d, io, ((EvalThread) io).activity);

                    case RUNONUI:
                        ConsList finalLs = ls;
                        ((EvalThread) io).activity.runOnUiThread(() -> {
                            eval(d, true, io, env, finalLs);
                        });
                    ...
                    case FIELD:
                        ...
                        if (v instanceof Class)
                            return getField((Class) v, null, name);
                        else
                            return getField(v.getClass(), v, name);
...
//                }
    ...
    public static Object evalLinkedList(InOutable io, Env env, ArrayDeque<Object> ls) {
        ...
        if (op instanceof SpecialForm) {
            ...
            switch (sf) {
                case ACTIVITY:
                    return ((EvalThread) io).activity;

                case RUNONUI:
                    ((EvalThread) io).activity.runOnUiThread(() -> {
                        evalIter(io, env, ls.getFirst());
                    });
                    return stringOK;
...
*/
    ...
    public static Object evalIter(InOutable io, Env env, Object inobj) {
...
//                                    break;
                    ...
                    if (si.p.isEmpty()) {
                        ...
                        if ((si.l.isEmpty() || isTailForm(si.l.getFirst()))
                            ...
                            maxstacksize =
                                    Math.max(stack.size(), maxstacksize);
                ...
                Object op = si.l.isEmpty() ? emptyList : si.l.getFirst();
                ...
                if (op instanceof Func) {
                    ...
                    Env funcEnv = new Env(getMapArgsValsNoeval(f.pars, si.l), f.clojure);
                    if (f.body instanceof Symbol) {
                        r = funcEnv.getVar(f.body.toString());
                    } else if (f.body instanceof ConsList) {
                        si.upd((ConsList) f.body, funcEnv);
                        continue;
                    } else r = f.body;
                    ...
                    if (me instanceof Symbol) {
                        r = si.env.getVar(me.toString());
                    } else if (me instanceof ConsList) {
                        si.upd((ConsList) me);
                        continue;
                    } else r = me;

                } else {
                    r = evalLinkedList(io, si.env, si.l);
                }
